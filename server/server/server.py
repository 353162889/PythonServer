...
class MsgPacket:
    ...
    def __init__(self, headBytes=None,packetID = None,packetStatus = None):
        self.head = headBytes
        self.serializeBytes = None
        self.buff = None
        self.proto = None
        if headBytes is not None:
            unpack = struct.unpack("iHH", self.head)
            self.packetSize = unpack[0]
            self.packetID = unpack[1]
            self.packetStatus = unpack[2]
        if packetID is not None:
            self.packetID = packetID
        if packetStatus is not None:
    ...
    def serialize(self):
        if self.serializeBytes is None:
            ...
            if self.buff is not None:
                ...
                self.packetSize = length + MsgPacket.HeadSize
                self.serializeBytes = struct.pack("iHH{0}s".format(length), self.packetSize, self.packetID,self.packetStatus,self.buff)
            else:
                self.packetSize = MsgPacket.HeadSize
                self.serializeBytes = struct.pack("iHH", self.packetSize,self.packetID,self.packetStatus)
...
class PacketReceiver:
    ...
    def __handle_packet(self):
        if self.dataLength >= self.packet.packetSize:
            ...
            self.bytesIO.seek(self.packet.packetSize)
            leaveBuff = self.bytesIO.read(self.dataLength - self.packet.packetSize)
            self.bytesIO.seek(0)
    ...
    def handMsg(self):
        while not self.receiver.packetQueue.empty():
            ...
            if pack.buff is not None:
                ...
                msg = info.msg
                print("receive from{0},id:{1} status:{2} data:{3}".format(self.id,pack.packetID,pack.packetStatus, info.msg))
            sendPack = MsgPacket(packetID=1,packetStatus=1)
            # returnMsg = "[server return]"+msg
            returnProto = protobuf.Msg_pb2.Info()
